...
- **`enable_thinking`으로 토글 통일** — thinking 분기를
  `{%- if enable_thinking is not defined or enable_thinking is true %}`(32B 기본값)로 일원화.
- `enable_thinking` **미지정 시 thinking ON**(`<think>` 생성)으로 동작한다.
  SEED-Think는 추론 튜닝 모델이라 사고를 강제로 끄면 tool-call 등 구조화 출력이
  깨지므로, 기본 ON이 맞다. `hcx_reasoner.py` 폴백 기본값도 ON으로 맞춰져 있다.
